    ...
    <div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6">
        <div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
            <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                <h1 class="page-heading d-flex text-dark fw-bold fs-3 flex-column justify-content-center my-0">Laporan Penerima Bantuan</h1>
            ...
            <div class="d-flex align-items-center gap-2 gap-lg-3">
                ...
                <a href="{{ route('reports.penerima_bantuan.print', request()->query()) }}" target="_blank" class="btn btn-sm fw-bold btn-primary">
                    Cetak Laporan PDF
                </a>
            </div>
        </div>
        ...
        <div id="kt_app_content_container" class="app-container container-xxl">
            ...
            <div class="card mb-8">
                ...
                <div class="card-body py-0 px-6">
                    <form action="{{ route('reports.penerima_bantuan.index') }}" method="GET" class="form">
                        ...
                        <div class="row g-9 mb-8">
                            ...
                            <div class="col-md-4 fv-row">
                                <label for="status_penerima" class="form-label fs-6 fw-bold">Status Penerima</label>
                                <select name="status_penerima" id="status_penerima" class="form-select form-select-solid fw-bold" data-control="select2" data-placeholder="Pilih Status" data-allow-clear="true">
                                    <option value=""></option>
                                    @foreach($statusPenerimaOptions as $value => $label)
                                        <option value="{{ $value }}" {{ $request->status_penerima == $value ? 'selected' : '' }}>
                                            {{ $label }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            {{-- Filter Petugas yang Ditugaskan --}}
                            
                        </div>

                        <div class="row g-9 mb-8">
                            <div class="col-md-12 text-end">
                                <a href="{{ route('reports.penerima_bantuan.index') }}" class="btn btn-light-primary me-3">Reset Filter</a>
                ...
                <div class="card-body pt-0">
                    <div class="table-responsive">
                        <table class="table table-striped table-hover table-row-bordered gy-5 gs-7">
                            ...
                            <tbody>
                                @forelse($penerimaBantuans as $index => $penerimaBantuan)
                                <tr>
                                    <td>{{ $penerimaBantuans->firstItem() + $index }}</td>
                                    <td>{{ $penerimaBantuan->keluarga->nik_kepala_keluarga ?? '-' }}</td>
                                    <td>{{ $penerimaBantuan->keluarga->nama_kepala_keluarga ?? '-' }}</td>
                                    <td>{{ $penerimaBantuan->keluarga->desa->nama_desa ?? '-' }}</td>
                                    <td>{{ $penerimaBantuan->jenisBantuan->nama_bantuan ?? '-' }}</td>
                                    <td>{{ \Carbon\Carbon::parse($penerimaBantuan->periode_bantuan)->translatedFormat('F Y') }}</td>
                                    <td>
                                        <span class="badge badge-light-{{ 
                                            $penerimaBantuan->status_penerima == 'tersalurkan' ? 'success' : 
                                            ($penerimaBantuan->status_penerima == 'belum_tersalurkan' ? 'warning' : 'info') 
                                            }}">{{ Str::title(str_replace('_', ' ', $penerimaBantuan->status_penerima)) }}</span>
                                    </td>
                                </tr>
                                @empty
                                <tr>
                                    <td colspan="9" class="text-center">Tidak ada data penerima bantuan yang ditemukan.</td>
                                </tr>
                                @endforelse
                            </tbody>
                        </table>
                    ...
                    <div class="d-flex justify-content-center mt-5">
                        {{ $penerimaBantuans->links('pagination::bootstrap-5') }} {{-- Sesuaikan pagination view Anda --}}
                    </div>
                </div>
            </div>
            {{-- End Card Tabel Data --}}
