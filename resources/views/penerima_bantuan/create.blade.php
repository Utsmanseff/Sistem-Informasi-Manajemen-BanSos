    ...
    <div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6">
        <div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
            <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                <h1 class="page-heading d-flex text-dark fw-bold fs-3 flex-column justify-content-center my-0">Tambah Penerima Bantuan</h1>
            ...
            <div class="card">
                <div class="card-header border-0 pt-6">
                    <div class="card-title">
                        <h2>Form Tambah Penerima Bantuan</h2>
                    </div>
                </div>
                <div class="card-body pt-0">
                    <form action="{{ route('penerima-bantuan.store') }}" method="POST" class="form">
                        @csrf
                            ...
                            <div class="col-md-6 fv-row">
                                ...
                                <select name="jenis_bantuan_id" id="jenis_bantuan_id" class="form-select @error('jenis_bantuan_id') is-invalid @enderror" data-control="select2" data-placeholder="Pilih Jenis Bantuan">
                                    ...
                                    @foreach($jenisBantuans as $jenisBantuan)
                                        <option value="{{ $jenisBantuan->id }}" data-nama="{{ $jenisBantuan->nama_bantuan }}" {{ old('jenis_bantuan_id') == $jenisBantuan->id ? 'selected' : '' }}>
                        ...
                        <div class="row g-9 mb-8" id="nominal_manual_section" style="display: none;"> 
                            <div class="col-md-6 fv-row">
                                ...
                                <input type="number" name="nominal_manual" id="nominal_manual" class="form-control @error('nominal_manual') is-invalid @enderror" value="{{ old('nominal_manual') }}" step="0.01" min="0">
                            ...
                            <div class="col-md-6 fv-row">
                                ...
                                <select name="periode_bantuan" id="periode_bantuan" class="form-select @error('periode_bantuan') is-invalid @enderror" data-control="select2" data-placeholder="Pilih Periode">
                                    ...
                                    @foreach($periods as $value => $label)
                                        <option value="{{ $value }}" {{ old('periode_bantuan') == $value ? 'selected' : '' }}>
                                            {{ $label }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-6 fv-row">
                                ...
                                <select name="ditugaskan_kepada_distributor_id" id="ditugaskan_kepada_distributor_id" class="form-select @error('ditugaskan_kepada_distributor_id') is-invalid @enderror" data-control="select2" data-placeholder="Pilih Distributor">
                                    ...
                                    @foreach($distributors as $distributor)
                                        <option value="{{ $distributor->id }}" {{ old('ditugaskan_kepada_distributor_id') == $distributor->id ? 'selected' : '' }}>
                                            {{ $distributor->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="row g-9 mb-8">
                            <div class="col-md-12 fv-row"> 
                                ...
                                <textarea name="catatan" id="catatan" class="form-control @error('catatan') is-invalid @enderror" rows="3" placeholder="Masukkan catatan tambahan">{{ old('catatan') }}</textarea>
                                @error('catatan')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
